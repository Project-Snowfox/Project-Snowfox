...
$isAllComments = ('on' == $request->get('__Snowfox_all_comments') || 'on' == \Snowfox\Cookie::get('__Snowfox_all_comments'));
?>
<div class="main">
    <div class="body container">
        ...
        <div class="row Snowfox-page-main" role="main">
            <div class="col-mb-12 Snowfox-list">
                <div class="clearfix">
                    <ul class="Snowfox-option-tabs right">
                    <?php if($user->pass('editor', true) && !isset($request->cid)): ?>
                        <li class="<?php if($isAllComments): ?> current<?php endif; ?>"><a href="<?php echo $request->makeUriByRequest('__Snowfox_all_comments=on'); ?>"><?php _e('所有'); ?></a></li>
                        <li class="<?php if(!$isAllComments): ?> current<?php endif; ?>"><a href="<?php echo $request->makeUriByRequest('__Snowfox_all_comments=off'); ?>"><?php _e('我的'); ?></a></li>
                    <?php endif; ?>
                    </ul>
                    <ul class="Snowfox-option-tabs">
                ...
                <div class="Snowfox-list-operate clearfix">
                    <form method="get">
                        <div class="operate">
                            <label><i class="sr-only"><?php _e('全选'); ?></i><input type="checkbox" class="Snowfox-table-select-all" /></label>
                ...
                <div class="Snowfox-table-wrap">
                    <table class="Snowfox-list-table">
                        ...
                        <?php else: ?>
                        <tr>
                            <td colspan="4"><h6 class="Snowfox-list-table-title"><?php _e('没有评论') ?></h6></td>
                        </tr>
                        <?php endif; ?>
                        </tbody>
...
$(document).ready(function () {
    ...
    function rememberScroll () {
        $(window).bind('beforeunload', function () {
            $.cookie('__Snowfox_comments_scroll', $('body').scrollTop());
        });
    }

    // 自动滚动
    (function () {
        var scroll = $.cookie('__Snowfox_comments_scroll');

        if (scroll) {
            $.cookie('__Snowfox_comments_scroll', null);
