...
<div class="main">
    <div class="body container">
        ...
        <div class="row Snowfox-page-main Snowfox-post-area" role="form">
                ...
                <div class="col-mb-12 col-tb-9" role="main">
                    <?php if ($post->draft): ?>
                        <?php if ($post->draft['cid'] != $post->cid): ?>
                            <?php $postModifyDate = new \Snowfox\Date($post->draft['modified']); ?>
                            <cite
                    ...
                    <?php $permalink = \Snowfox\Common::url($options->routingTable['post']['url'], $options->index);
                    ...
                    <?php \Snowfox\Plugin::factory('admin/write-post.php')->call('content', $post); ?>
                </div>

                <div id="edit-secondary" class="col-mb-12 col-tb-3" role="complementary">
                    <ul class="Snowfox-option-tabs clearfix">
                    ...
                    <div id="tab-advance" class="tab-content">
                        <section class="Snowfox-post-option" role="application">
                            <label for="date" class="Snowfox-label"><?php _e('发布日期'); ?></label>
                            <p><input class="Snowfox-date w-100" type="text" name="date" id="date" autocomplete="off"
                                      value="<?php $post->have() && $post->created > 0 ? $post->date('Y-m-d H:i') : ''; ?>"/>
                            </p>
                        </section>

                        <section class="Snowfox-post-option category-option">
                            <label class="Snowfox-label"><?php _e('分类'); ?></label>
                        ...
                        <section class="Snowfox-post-option">
                            <label for="token-input-tags" class="Snowfox-label"><?php _e('标签'); ?></label>
                            <p><input id="tags" name="tags" type="text" value="<?php $post->have() ? $post->tags(',', false) : ''; ?>"
                                      class="w-100 text"/></p>
                        </section>

                        <?php \Snowfox\Plugin::factory('admin/write-post.php')->call('option', $post); ?>

                        <button type="button" id="advance-panel-btn" class="btn btn-xs"><?php _e('高级选项'); ?> <i
                                class="i-caret-down"></i></button>
                        <div id="advance-panel">
                            <?php if ($user->pass('editor', true)): ?>
                                <section class="Snowfox-post-option visibility-option">
                                    <label for="visibility" class="Snowfox-label"><?php _e('公开度'); ?></label>
                                    <p>
                            ...
                            <section class="Snowfox-post-option allow-option">
                                <label class="Snowfox-label"><?php _e('权限控制'); ?></label>
                                <ul>
                            ...
                            <section class="Snowfox-post-option">
                                <label for="trackback" class="Snowfox-label"><?php _e('引用通告'); ?></label>
                                <p><textarea id="trackback" class="w-100 mono" name="trackback" rows="2"></textarea></p>
                                <p class="description"><?php _e('每一行一个引用地址, 用回车隔开'); ?></p>
                            </section>

                            <?php \Snowfox\Plugin::factory('admin/write-post.php')->call('advanceOption', $post); ?>
                        </div><!-- end #advance-panel -->

                        <?php if ($post->have()): ?>
                            <?php $modified = new \Snowfox\Date($post->modified); ?>
                            <section class="Snowfox-post-option">
                                <p class="description">
                                    <br>&mdash;<br>
                                    <?php _e('本文由 <a href="%s">%s</a> 撰写',
                                        \Snowfox\Common::url('manage-posts.php?uid=' . $post->author->uid, $options->adminUrl), $post->author->screenName); ?>
                                    <br>
...
\Snowfox\Plugin::factory('admin/write-post.php')->trigger($plugged)->call('richEditor', $post);
if (!$plugged) {
...
\Snowfox\Plugin::factory('admin/write-post.php')->call('bottom', $post);
include 'footer.php';
?>
