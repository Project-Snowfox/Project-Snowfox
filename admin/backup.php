...
$actionUrl = $security->getTokenUrl(
    \Snowfox\Router::url('do', array('action' => 'backup', 'widget' => 'Backup'),
        \Snowfox\Common::url('index.php', $options->rootUrl)));
        ...
        <div class="row Snowfox-page-main" role="main">
            ...
            <div id="backup-secondary" class="col-mb-12 col-tb-4" role="form">
                ...
                <form action="<?php echo $actionUrl; ?>" id="from-server" class="tab-content hidden" method="post">
                    ...
                    <ul class="Snowfox-option">
                        <li>
                            <p class="description"><?php _e('将备份文件手动上传至服务器的 %s 目录下后, 这里会出现文件选项', __Snowfox_BACKUP_DIR__); ?></p>
                        </li>
                    </ul>
                    <?php else: ?>
                    <ul class="Snowfox-option">
                        <li>
                            <label class="Snowfox-label" for="backup-select-file"><?php _e('选择一个备份文件恢复数据'); ?></label>
...
?>
<script>
    $('#backup-secondary .Snowfox-option-tabs li').click(function() {
        $('#backup-secondary .Snowfox-option-tabs li').removeClass('active');
