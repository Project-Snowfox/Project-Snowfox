...
class Form extends Layout
{
    ...
    public function validate(): array
    {
        ...
        if ($error) {
            /** 利用session记录错误 */
            Cookie::set('__Snowfox_form_message_' . $id, json_encode($error));

            /** 利用session记录表单值 */
            Cookie::set('__Snowfox_form_record_' . $id, json_encode($formData));
        }
    ...
    {
        $id = md5(implode('"', array_keys($this->inputs)));
        $record = Cookie::get('__Snowfox_form_record_' . $id);
        $message = Cookie::get('__Snowfox_form_message_' . $id);
        ...
        if (!empty($record)) {
            ...
            Cookie::delete('__Snowfox_form_record_' . $id);
        }

        parent::render();
        Cookie::delete('__Snowfox_form_message_' . $id);
    }
}
