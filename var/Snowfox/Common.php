...
namespace Snowfox {
    const PLUGIN_NAMESPACE = 'SnowfoxPlugin';

    spl_autoload_register(function (string $className) {
        $isDefinedAlias = defined('__Snowfox_CLASS_ALIASES__');
        $isNamespace = strpos($className, '\\') !== false;
        $isAlias = $isDefinedAlias && isset(__Snowfox_CLASS_ALIASES__[$className]);
        $isPlugin = false;
        ...
        if ($isNamespace) {
            ...
            if ($isPlugin) {
                ...
                if ($isDefinedAlias) {
                    $alias = array_search('\\' . ltrim($className, '\\'), __Snowfox_CLASS_ALIASES__);
                }
            ...
            $isPlugin = !$isAlias && !preg_match("/^(Snowfox|Widget|IXR)_/", $className);

            if ($isPlugin) {
                $alias = '\\SnowfoxPlugin\\' . str_replace('_', '\\', $className);
                $path = str_replace('_', '/', $className);
            } else {
                $alias = $isAlias ? __Snowfox_CLASS_ALIASES__[$className]
        ...
        // load class file
        $path .= '.php';
        $defaultFile = __Snowfox_ROOT_DIR__ . '/var/' . $path;

        if (file_exists($defaultFile) && !$isPlugin) {
            include_once $defaultFile;
        } else {
            $pluginFile = __Snowfox_ROOT_DIR__ . __Snowfox_PLUGIN_DIR__ . '/' . $path;
    ...
    class Common
    {
        ...
        public static function error(\Throwable $exception)
        {
            ...
            if ($exception instanceof \Snowfox\Db\Exception) {
                $code = 500;

                //覆盖原始错误信息
                $message = 'Database Server Error';

                if ($exception instanceof \Snowfox\Db\Adapter\ConnectionException) {
                    $code = 503;
                    $message = 'Error establishing a database connection';
                } elseif ($exception instanceof \Snowfox\Db\Adapter\SQLException) {
                    $message = 'Database Query Error';
                }
            } elseif ($exception instanceof \Snowfox\Widget\Exception) {
                $message = $exception->getMessage();
            } else {
            ...
            if (defined('__Snowfox_EXCEPTION_FILE__')) {
                require_once __Snowfox_EXCEPTION_FILE__;
            } else {
                echo
                <<<EOF
<!DOCTYPE html>
...
EOF;
        ...
        public static function gravatarUrl(
            ...
            if (defined('__Snowfox_GRAVATAR_PREFIX__')) {
                $url = __Snowfox_GRAVATAR_PREFIX__;
            } else {
