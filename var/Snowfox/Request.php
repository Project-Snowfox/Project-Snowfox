...
class Request
{
    ...
    public function getPathInfo(): ?string
    {
        ...
        if (!empty($pathInfo)) {
            //针对iis的utf8编码做强制转换
            $pathInfo = defined('__Snowfox_PATHINFO_ENCODING__') ?
                mb_convert_encoding($pathInfo, 'UTF-8', __Snowfox_PATHINFO_ENCODING__) : $pathInfo;
        } else {
            $pathInfo = '/';
        }
    ...
    {
        if (null === $this->ip) {
            $header = defined('__Snowfox_IP_SOURCE__') ? __Snowfox_IP_SOURCE__ : 'X-Forwarded-For';
    ...
    public function isSecure(): bool
    {
        return (!empty($_SERVER['HTTP_X_FORWARDED_PROTO']) && !strcasecmp('https', $_SERVER['HTTP_X_FORWARDED_PROTO']))
            ...
            || (defined('__Snowfox_SECURE__') && __Snowfox_SECURE__);
    }
    ...
    public function getUrlPrefix(): ?string
    {
        if (empty($this->urlPrefix)) {
            if (defined('__Snowfox_URL_PREFIX__')) {
                $this->urlPrefix = __Snowfox_URL_PREFIX__;
