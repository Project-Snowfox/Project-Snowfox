...
{
    public const HEADER = '%Snowfox_BACKUP_XXXX%';
    ...
    {
        return array_map('basename', glob(__Snowfox_BACKUP_DIR__ . '/*.dat'));
    }
    ...
    private function import()
    {
        ...
        if (!empty($_FILES)) {
            ...
            $path = __Snowfox_BACKUP_DIR__ . '/' . $this->request->get('file');
    ...
    private function parseHeader($str, &$version): bool
    {
        ...
        if (!preg_match("/%Snowfox_BACKUP_[A-Z0-9]{4}%/", $str)) {
            return false;
        }
    ...
    private function reLogin(&$user)
    {
        ...
        Cookie::set('__Snowfox_uid', $user['uid']);
        Cookie::set('__Snowfox_authCode', Common::hash($user['authCode']));
        $this->login = true;
    }
}
