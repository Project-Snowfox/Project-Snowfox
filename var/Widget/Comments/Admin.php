...
class Admin extends Comments
{
    ...
    public function execute()
    {
        ...
        if (!$this->user->pass('editor', true)) {
            ...
            if ($this->request->is('__Snowfox_all_comments=on')) {
                Cookie::set('__Snowfox_all_comments', 'on');
            } else {
                if ($this->request->is('__Snowfox_all_comments=off')) {
                    Cookie::set('__Snowfox_all_comments', 'off');
                }

                if ('on' != Cookie::get('__Snowfox_all_comments')) {
                    $select->where('table.comments.ownerId = ?', $this->user->uid);
                }
            }
        }
