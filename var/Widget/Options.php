...
use Snowfox\Widget;
use Snowfox\Plugin\Exception as PluginException;
...
class Options extends Base
{
    ...
    public function execute()
    {
        ...
        if (isset($this->db)) {
            ...
            // finish install
            if (empty($values)) {
                $this->response->redirect(defined('__Snowfox_ADMIN__')
    ...
    {
        return __Snowfox_ROOT_DIR__ . __Snowfox_THEME_DIR__ . '/' . trim($theme, './') . '/' . trim($file, './');
    }
    ...
    public function themeUrl(?string $path = null, ?string $theme = null)
    {
        if (!isset($theme)) {
            ...
            $url = defined('__Snowfox_THEME_URL__') ? __Snowfox_THEME_URL__ :
                Common::url(__Snowfox_THEME_DIR__ . '/' . $theme, $this->siteUrl);
    ...
    {
        $rootUrl = defined('__Snowfox_ROOT_URL__') ? __Snowfox_ROOT_URL__ : $this->request->getRequestRoot();

        if (defined('__Snowfox_ADMIN__')) {
            /** 识别在admin目录中的情况 */
            $adminDir = '/' . trim(defined('__Snowfox_ADMIN_DIR__') ? __Snowfox_ADMIN_DIR__ : '/admin/', '/');
    ...
    protected function ___originalSiteUrl(): string
    {
        ...
        if (defined('__Snowfox_SITE_URL__')) {
            $siteUrl = __Snowfox_SITE_URL__;
        } elseif (defined('__Snowfox_DYNAMIC_SITE_URL__') && __Snowfox_DYNAMIC_SITE_URL__) {
    ...
    {
        return ($this->rewrite || (defined('__Snowfox_REWRITE__') && __Snowfox_REWRITE__))
    ...
    {
        return defined('__Snowfox_PLUGIN_URL__') ? __Snowfox_PLUGIN_URL__ :
            Common::url(__Snowfox_PLUGIN_DIR__, $this->siteUrl);
    }
    ...
    {
        return Common::url(__Snowfox_PLUGIN_DIR__, __Snowfox_ROOT_DIR__);
    }
    ...
    {
        return Common::url(defined('__Snowfox_ADMIN_DIR__') ?
            __Snowfox_ADMIN_DIR__ : '/admin/', $this->rootUrl);
    }
