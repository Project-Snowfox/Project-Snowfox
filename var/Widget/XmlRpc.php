...
use Snowfox\Router;
use Snowfox\Widget;
use Snowfox\Widget\Exception as WidgetException;
...
class XmlRpc extends Contents implements ActionInterface, Hook
{
    ...
    public function wpGetPage(int $blogId, int $pageId, string $userName, string $password): array
    {
        ...
        return [
            ...
            'page_id'                => $page->cid,
            'page_status'            => $this->SnowfoxToWordpressStatus($page->status, 'page'),
            'description'            => $excerpt,
            'title'                  => $page->title,
            'link'                   => $page->permalink,
    ...
    public function wpGetPages(int $blogId, string $userName, string $password): array
    {
        ...
        while ($pages->next()) {
            ...
            $pageStructs[] = [
                ...
                'page_status'            => $this->SnowfoxToWordpressStatus(
    ...
    public function mwNewPost(int $blogId, string $userName, string $password, array $content, bool $publish): int
    {
        ...
        if (isset($content["{$type}_status"])) {
            $status = $this->wordpressToSnowfoxStatus($content["{$type}_status"], $type);
    ...
    public function wpGetComment(int $blogId, string $userName, string $password, int $commentId): array
    {
        ...
        return [
            ...
            'status'           => $this->SnowfoxToWordpressStatus($comment->status, 'comment'),
    ...
    public function wpGetComments(int $blogId, string $userName, string $password, array $struct): array
    {
        ...
        if (!empty($struct['status'])) {
            $input['status'] = $this->wordpressToSnowfoxStatus($struct['status'], 'comment');
        } else {
            $input['__Snowfox_all_comments'] = 'on';
        }
        ...
        while ($comments->next()) {
            $commentsStruct[] = [
                ...
                'status'           => $this->SnowfoxToWordpressStatus($comments->status, 'comment'),
    ...
    public function mwGetPost(int $postId, string $userName, string $password): array
    {
        ...
        return [
            ...
            'post_status'            => $this->SnowfoxToWordpressStatus($post->status, 'post'),
            'custom_fields'          => [],
            'sticky'                 => 0
        ];
    }
    ...
    public function mwGetRecentPosts(int $blogId, string $userName, string $password, int $postsNum): array
    {
        ...
        while ($posts->next()) {
            ...
            $postStructs[] = [
                ...
                'post_status'            => $this->SnowfoxToWordpressStatus(
...
EOF;
    ...
    /**
     * 将Snowfox的状态类型转换为wordperss的风格
     *
     * @param string $status Snowfox的状态
     * @param string $type 内容类型
     * @return string
     */
    private function SnowfoxToWordpressStatus(string $status, string $type = 'post'): string
    {
        if ('post' == $type) {
            /** 文章状态 */
            switch ($status) {
     ...
     * @return string
     */
    private function wordpressToSnowfoxStatus(string $status, string $type = 'post'): string
    {
        if ('post' == $type) {
            /** 文章状态 */
            switch ($status) {
