...
class Admin extends Contents
{
    ...
    public function execute()
    {
        ...
        if (!$this->user->pass('editor', true)) {
            ...
            if ($this->request->is('__Snowfox_all_posts=on')) {
                Cookie::set('__Snowfox_all_posts', 'on');
            } else {
                if ($this->request->is('__Snowfox_all_posts=off')) {
                    Cookie::set('__Snowfox_all_posts', 'off');
                }

                if ('on' != Cookie::get('__Snowfox_all_posts')) {
                    $select->where(
