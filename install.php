...
if (!file_exists(dirname(__FILE__) . '/config.inc.php')) {
    // site root path
    define('__Snowfox_ROOT_DIR__', dirname(__FILE__));

    // plugin directory (relative path)
    define('__Snowfox_PLUGIN_DIR__', '/usr/plugins');

    // theme directory (relative path)
    define('__Snowfox_THEME_DIR__', '/usr/themes');

    // admin directory (relative path)
    define('__Snowfox_ADMIN_DIR__', '/admin/');

    // register autoload
    require_once __Snowfox_ROOT_DIR__ . '/var/Snowfox/Common.php';

    // init
    \Snowfox\Common::init();
} else {
    require_once dirname(__FILE__) . '/config.inc.php';
    $installDb = \Snowfox\Db::get();
}
...
{
    $serverLang = \Snowfox\Request::getInstance()->getServer('Snowfox_LANG');

    if (!empty($serverLang)) {
        return $serverLang;
    } else {
        $lang = 'zh_CN';
        $request = \Snowfox\Request::getInstance();

        if ($request->is('lang')) {
            $lang = $request->get('lang');
            \Snowfox\Cookie::set('lang', $lang);
        }

        return \Snowfox\Cookie::get('lang', $lang);
    }
}
...
{
    $request = \Snowfox\Request::getInstance();
    return install_is_cli() ? $request->getServer('Snowfox_SITE_URL', 'http://localhost') : $request->getRequestRoot();
}
...
{
    return \Snowfox\Request::getInstance()->isCli();
}
...
function install_get_default_options(): array
{
    ...
    if (empty($options)) {
        $options = [
            'theme' => 'joe',
            'theme:joe' => json_encode([
              "JFavicon" => "https://cdn.aurorum.co/project-snowfox/theme-joe/img/favicon.png",
              "JLogo" => "https://cdn.aurorum.co/project-snowfox/theme-joe/img/icon.png",
              "JAssetsURL" => null,
              "JCommentStatus" => "on",
              "JNavMaxNum" => "3",
              "JCustomNavs" => null,
              "JList_Animate" => "off",
              "JFooter_Left" => "Powered by Snowfox",
              "JFooter_Right" => "SiteMap",
              "JDocumentTitle" => null,
              "JCursorEffects" => "off",
              "JCustomCSS" => null,
              "JCustomScript" => null,
              ...
              "JBirthDay" => null,
              "JCustomFont" => null,
              "JCustomAvatarSource" => null,
              "JAside_Author_Nick" => "Snowfox",
              "JAside_Author_Avatar" => null,
              "JAside_Author_Image" => "https://npm.elemecdn.com/Snowfox-joe-latest/assets/img/aside_author_image.jpg",
              "JAside_Wap_Image" => "https://npm.elemecdn.com/Snowfox-joe-latest/assets/img/wap_aside_image.jpg",
              "JAside_Author_Link" => "https://78.al",
              "JAside_Author_Motto" => "有钱终成眷属，没钱亲眼目睹",
              "JAside_Author_Nav" => "off",
              "JAside_Timelife_Status" => "off",
              "JAside_Hot_Num" => "off",
              "JAside_Newreply_Status" => "off",
              "JAside_Weather_Key" => null,
              "JAside_Weather_Style" => "1",
              "JADContent" => null,
              "JCustomAside" => null,
              "JAside_3DTag" => "off",
              "JAside_Flatterer" => "off",
              "JAside_History_Today" => "off",
              "JThumbnail" => null,
              "JLazyload" => "https://npm.elemecdn.com/Snowfox-joe-latest/assets/img/lazyload.jpg",
              "JDynamic_Background" => "off",
              "JWallpaper_Background_PC" => null,
              "JWallpaper_Background_WAP" => null,
              "JIndex_Carousel" => null,
              "JIndex_Recommend" => null,
              "JIndexSticky" => null,
              "JIndex_Hot" => "off",
              "JIndex_Ad" => null,
              "JIndex_Notice" => null,
              "JFriends" => "Joe的博客 || https://78.al || https://npm.elemecdn.com/Snowfox-joe-latest/assets/img/link.png || Eternity is not a distance but a decision",
              "JMaccmsAPI" => null,
              "JCustomPlayer" => null,
              "JSensitiveWords" => "你妈死了 || 傻逼 || 操你妈 || 射你妈一脸",
              "JLimitOneChinese" => "off",
              "JTextLimit" => null,
              "JSiteMap" => "off",
              "JBTPanel" => null,
              "JBTKey" => null,
              "JCommentMail" => "off",
              "JCommentMailHost" => null,
              "JCommentSMTPSecure" => "ssl",
              "JCommentMailPort" => null,
              "JCommentMailFromName" => null,
              "JCommentMailAccount" => null,
              "JCommentMailPassword" => null,
              "JBaiduToken" => null,
              "JOverdue" => "off",
              "JEditor" => "on",
              "JPrismTheme" => "//npm.elemecdn.com/prismjs@1.29.0/themes/prism.min.css"
            ]),
            'timezone' => '28800',
            ...
            'gzip' => 0,
            'generator' => 'Snowfox ' . \Snowfox\Common::VERSION,
            'title' => 'Hello World',
            'description' => 'Your description here.',
            'keywords' => 'Snowfox,php,blog',
            'rewrite' => 0,
            'frontPage' => 'recent',
            'frontArchive' => 0,
            ...
            'secret' => \Snowfox\Common::randString(32, true),
            'installed' => 0,
            'allowXmlRpc' => 2
        ];
    }
...
function install_get_db_drivers(): array
{
    ...
    if (\Snowfox\Db\Adapter\Pdo\Mysql::isAvailable()) {
        $drivers['Pdo_Mysql'] = _t('Pdo 驱动 Mysql 适配器');
    }

    if (\Snowfox\Db\Adapter\Pdo\SQLite::isAvailable()) {
        $drivers['Pdo_SQLite'] = _t('Pdo 驱动 SQLite 适配器');
    }

    if (\Snowfox\Db\Adapter\Pdo\Pgsql::isAvailable()) {
        $drivers['Pdo_Pgsql'] = _t('Pdo 驱动 PostgreSql 适配器');
    }

    if (\Snowfox\Db\Adapter\Mysqli::isAvailable()) {
        $drivers['Mysqli'] = _t('Mysql 原生函数适配器');
    }

    if (\Snowfox\Db\Adapter\SQLite::isAvailable()) {
        $drivers['SQLite'] = _t('SQLite 原生函数适配器');
    }

    if (\Snowfox\Db\Adapter\Pgsql::isAvailable()) {
...
function install_get_current_db_driver(): string
{
    ...
    if (empty($installDb)) {
        $driver = \Snowfox\Request::getInstance()->get('driver');
...
// config db
\$db = new \Snowfox\Db('{$adapter}', '{$dbPrefix}');
\$db->addServer(" . (var_export($dbConfig, true)) . ", \Snowfox\Db::READ | \Snowfox\Db::WRITE);
\Snowfox\Db::set(\$db);
";
    ...
    if (!$return) {
        $configWritten = @file_put_contents(__Snowfox_ROOT_DIR__ . '/config.inc.php', $code) !== false;
    }
...
function install_remove_config_file()
{
    ...
    if ($configWritten) {
        unlink(__Snowfox_ROOT_DIR__ . '/config.inc.php');
    }
}
...
function install_check(string $type): bool
{
    switch ($type) {
        case 'config':
            return file_exists(__Snowfox_ROOT_DIR__ . '/config.inc.php');
        case 'db_structure':
        case 'db_data':
            global $installDb;
            ...
            } catch (\Snowfox\Db\Adapter\ConnectionException $e) {
                return true;
            } catch (\Snowfox\Db\Adapter\SQLException $e) {
                return false;
            }
...
{
    \Snowfox\Response::getInstance()->setContentType('application/json')
...
{
    \Snowfox\Response::getInstance()->setStatus(302)
...
{
    ?>
    <div id="success" class="row Snowfox-page-main hidden">
        <div class="col-mb-12 col-tb-8 col-tb-offset-2">
            <div class="Snowfox-page-title">
                <h2><?php _e('安装成功'); ?></h2>
            </div>
            <div id="Snowfox-welcome">
                <p class="keep-word">
                ...
                <p><?php _e('希望您能尽情享用 Snowfox 带来的乐趣!'); ?></p>
            </div>
        </div>
    </div>
    <script>
        ...
        form.submit(function (e) {
            ...
            $('.Snowfox-option .error', form).remove();
                    ...
                    if (data.success) {
                        if (data.message) {
                            ...
                            if (data.config) {
                                ...
                                $('.Snowfox-page-main:first').addClass('hidden');
...
function install_step_1()
{
    ...
    $lang = install_get_lang();
    ?>
    <div class="row Snowfox-page-main">
        <div class="col-mb-12 col-tb-8 col-tb-offset-2">
            <div class="Snowfox-page-title">
                <h2><?php _e('欢迎使用 Snowfox'); ?></h2>
            </div>
            <div id="Snowfox-welcome">
                <form autocomplete="off" method="post" action="install.php">
                    ...
                    <h3><?php _e('许可及协议'); ?></h3>
                    <ul>
                        <li><?php _e('Snowfox 基于 <a href="https://www.gnu.org/copyleft/gpl.html">GPL</a> 协议发布, 我们允许用户在 GPL 协议许可的范围内使用, 拷贝, 修改和分发此程序.'); ?>
                            <?php _e('在GPL许可的范围内, 您可以自由地将其用于商业以及非商业用途.'); ?></li>
                        <li><?php _e('Snowfox 软件由其社区提供支持, 核心开发团队负责维护程序日常开发工作以及新特性的制定.'); ?>
...
function install_step_1_perform()
{
    ...
    $realUploadDir = \Snowfox\Common::url($uploadDir, __Snowfox_ROOT_DIR__);
    $writeable = true;
...
function install_step_2()
{
    ...
    if (!empty($installDb)) {
        $config = $installDb->getConfig(\Snowfox\Db::WRITE)->toArray();
        ...
        <div class="col-mb-12 col-tb-8 col-tb-offset-2">
            ...
            <form autocomplete="off" action="install.php" method="post">
                <ul class="Snowfox-option">
                    <li>
                        <label for="dbAdapter" class="Snowfox-label"><?php _e('数据库适配器'); ?></label>
                ...
                <ul class="Snowfox-option">
                    <li>
                        <label class="Snowfox-label" for="dbPrefix"><?php _e('数据库前缀'); ?></label>
                        <input type="text" class="text" name="dbPrefix" id="dbPrefix" value="Snowfox_" />
                        <p class="description"><?php _e('默认前缀是 "Snowfox_"'); ?></p>
                    </li>
                </ul>
                <?php require_once './install/' . $type . '.php'; ?>


                <ul class="Snowfox-option Snowfox-option-submit">
                    <li>
...
function install_step_2_perform()
{
    ...
    $configMap = [
        ...
        'SQLite' => [
            'dbFile' => __Snowfox_ROOT_DIR__ . '/usr/' . uniqid() . '.db'
        ]
    ];

    if (install_is_cli()) {
        $config = [
            'dbHost' => $request->getServer('Snowfox_DB_HOST'),
            'dbUser' => $request->getServer('Snowfox_DB_USER'),
            'dbPassword' => $request->getServer('Snowfox_DB_PASSWORD'),
            'dbCharset' => $request->getServer('Snowfox_DB_CHARSET'),
            'dbPort' => $request->getServer('Snowfox_DB_PORT'),
            'dbDatabase' => $request->getServer('Snowfox_DB_DATABASE'),
            'dbFile' => $request->getServer('Snowfox_DB_FILE'),
            'dbDsn' => $request->getServer('Snowfox_DB_DSN'),
            'dbEngine' => $request->getServer('Snowfox_DB_ENGINE'),
            'dbPrefix' => $request->getServer('Snowfox_DB_PREFIX', 'Snowfox_'),
            'dbAdapter' => $request->getServer('Snowfox_DB_ADAPTER', install_get_current_db_driver()),
            'dbNext' => $request->getServer('Snowfox_DB_NEXT', 'none'),
            'dbSslCa' => $request->getServer('Snowfox_DB_SSL_CA'),
            'dbSslVerify' => $request->getServer('Snowfox_DB_SSL_VERIFY', 'off'),
        ];
    } else {
        $config = $request->from([
            'dbHost',
    ...
    if ($config['dbNext'] == 'config' && !install_check('config')) {
        ...
        // detect db config
        try {
            $installDb = new \Snowfox\Db($config['dbAdapter'], $config['dbPrefix']);
            $installDb->addServer($dbConfig, \Snowfox\Db::READ | \Snowfox\Db::WRITE);
    ...
    // init db structure
    try {
        $scripts = file_get_contents(__Snowfox_ROOT_DIR__ . '/install/' . $type . '.sql');
        $scripts = str_replace('Snowfox_', $config['dbPrefix'], $scripts);
        ...
        foreach ($scripts as $script) {
            $script = trim($script);
            if ($script) {
                $installDb->query($script, \Snowfox\Db::WRITE);
            }
        }
    } catch (\Snowfox\Db\Exception $e) {
        $code = $e->getCode();
    ...
    <div class="row Snowfox-page-main">
        <div class="col-mb-12 col-tb-8 col-tb-offset-2">
            ...
            <form autocomplete="off" action="install.php" method="post">
                <ul class="Snowfox-option">
                    <li>
                        <label class="Snowfox-label" for="userUrl"><?php _e('网站地址'); ?></label>
                ...
                <ul class="Snowfox-option">
                    <li>
                        <label class="Snowfox-label" for="userName"><?php _e('用户名'); ?></label>
                ...
                <ul class="Snowfox-option">
                    <li>
                        <label class="Snowfox-label" for="userPassword"><?php _e('登录密码'); ?></label>
                ...
                <ul class="Snowfox-option">
                    <li>
                        <label class="Snowfox-label" for="userMail"><?php _e('邮件地址'); ?></label>
...
function install_step_3_perform()
{
    ...
    $request = \Snowfox\Request::getInstance();
    $defaultPassword = \Snowfox\Common::randString(8);
    $options = \Widget\Options::alloc();

    if (install_is_cli()) {
        $config = [
            'userUrl' => $request->getServer('Snowfox_SITE_URL'),
            'userName' => $request->getServer('Snowfox_USER_NAME', 'Snowfox'),
            'userPassword' => $request->getServer('Snowfox_USER_PASSWORD'),
            'userMail' => $request->getServer('Snowfox_USER_MAIL', 'admin@localhost.local')
        ];
    } else {
        $config = $request->from([
            'userUrl',
    ...
    try {
        ...
        $installDb->query(
            $installDb->insert('table.contents')->rows([
                'title' => _t('欢迎使用 Snowfox'),
                'slug' => 'start', 'created' => \Snowfox\Date::time(),
                'modified' => \Snowfox\Date::time(),
        ...
        $installDb->query(
            $installDb->insert('table.contents')->rows([
                ...
                'created' => \Snowfox\Date::time(),
                'modified' => \Snowfox\Date::time(),
                'text' => '<!--markdown-->' . _t('本页面由 Snowfox 创建, 这只是个测试页面.'),
                'authorId' => 1,
                'order' => 0,
                'type' => 'page',
                'status' => 'publish',
        ...
        $installDb->query(
            $installDb->insert('table.comments')->rows([
                'cid' => 1, 'created' => \Snowfox\Date::time(),
                'author' => 'Snowfox',
                'ownerId' => 1,
                'url' => 'https://typecho.org',
                'ip' => '127.0.0.1',
                'agent' => $options->generator,
                'text' => _t('欢迎加入 Snowfox 大家族'),
                'type' => 'comment',
                'status' => 'approved',
                'parent' => 0
            ])
    ...
    $loginUrl = \Snowfox\Common::buildUrl($parts);
...
function install_dispatch()
{
    ...
    if (install_is_cli()) {
        define('__Snowfox_ROOT_URL__', 'http://localhost');
    }
...
<head>
    ...
    <title><?php _e('Snowfox 安装程序'); ?></title>
...
<body>
    <div class="body container">
        <h1><a href="https://typecho.org" target="_blank" class="i-logo">Snowfox</a></h1>
        <?php $method(); ?>
    </div>
</body>
</html>
